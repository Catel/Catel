namespace Catel.MVVM
{
    ...
    public static class IViewModelFactoryExtensions
    {
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="viewModelFactory" /> is <c>null</c>.</exception>
        /// <exception cref="ArgumentException">The <c>TViewModel</c> does not implement the <see cref="IViewModel" /> interface.</exception>
        public static TViewModel? CreateViewModel<TViewModel>(this IViewModelFactory viewModelFactory, params object?[] args)
            where TViewModel : IViewModel
        {
            ArgumentNullException.ThrowIfNull(viewModelFactory);

            var viewModelType = typeof(TViewModel);
            return (TViewModel?)viewModelFactory.CreateViewModel(viewModelType, args);
        }
        ...
        public static IViewModel CreateRequiredViewModel(this IViewModelFactory viewModelFactory, Type viewModelType, object? dataContext)
        {
            ...
            if (vm is null)
            {
                throw new CatelException($"Cannot create view model '{viewModelType.FullName}'");
            }
        ...
        where TViewModel : IViewModel
        {
            ...
            if (vm is null)
            {
                throw new CatelException($"Cannot create view model '{typeof(TViewModel).FullName}'");
            }
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="viewModelFactory" /> is <c>null</c>.</exception>
        /// <exception cref="ArgumentException">The <c>TViewModel</c> does not implement the <see cref="IViewModel" /> interface.</exception>
        public static TViewModel CreateRequiredViewModel<TViewModel>(this IViewModelFactory viewModelFactory, params object?[] args)
        where TViewModel : IViewModel
        {
            ArgumentNullException.ThrowIfNull(viewModelFactory);

            var vm = viewModelFactory.CreateViewModel<TViewModel>(args);
            if (vm is null)
            {
                throw new CatelException($"Cannot create view model '{typeof(TViewModel).FullName}'");
            }
