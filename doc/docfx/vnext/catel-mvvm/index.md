...
title: "Catel.MVVM" 
---
The last few years, MVVM has become the number one pattern to write applications using WPF and other XAML related platforms. The actual pattern is simple, but there are some flaws and questions many MVVM users have, such as:
...
Before we actually started developing Catel, we did many investigations to ensure that the MVVM pattern was really useful in Line of Business (LoB) applications and does not miss the finishing touch. Thanks to this investigation and research, we created a solid MVVM framework which solves all the known problems of the MVVM pattern.
...
- easy to use, a view model is created within 10 minutes
