...
Catel is an application development platform with the focus on MVVM (WPF). The goal of Catel is to provide a complete set of modular functionality for Line of Business applications written in any .NET technology, from client to server.

Catel consists of 2 sub-projects:

- Catel.Core => Core library with generic software development functionality.
- Catel.MVVM => WPF library containing the MVVM features of Catel.
...
```
Catel.Core => Cross platform code
```

```
Catel.MVVM => Windows / WPF specific
```
